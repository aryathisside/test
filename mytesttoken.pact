(namespace "free")
(define-keyset "free.testtoken-admin" (read-keyset "testtoken-admin"))
(module mytesttoken GOVERNANCE

  @doc "testtoken smart contract"

  @model
    [ (defproperty conserves-mass (amount:decimal)
        (= (column-delta tkn-ledger 'balance) 0.0))
  ...
  (defschema tkn-schema
    balance:decimal
    guard:guard
  )
  (deftable tkn-ledger:{tkn-schema})

  ; --------------------------------------------------------------------------
  ; Capabilities
  (defcap GOVERNANCE()
    @doc " Give the admin full access to call and upgrade the module. "
    (enforce-keyset "free.testtoken-admin")
  )

  (defcap IS_ADMIN ()
    @doc " Capability for ADMIN specified functions "
    (compose-capability (GOVERNANCE))
  )

  (defcap DEBIT ( sender:string )
    @doc " Capability to perform debiting operations. "
    (enforce-guard (at 'guard (read tkn-ledger sender ['guard])))
  ...
  (defconst TKN_OWNER_ACC:string "TKN_OWNER"
  ...
  (defconst TKN_SUPPLY:decimal 10000000000.0
    " Initial supply of 10 Billion tokens." )

  (defun tkn-guard () (keyset-ref-guard "free.testtoken-admin"))

  (defconst DECIMALS 10
  ...
  (defun debit
    ...
    (with-read tkn-ledger accountId
      { "balance" := balance }

      (enforce (<= amount balance) "Insufficient funds.")

      (update tkn-ledger accountId
  ...
  (defun credit
    ...
    (with-default-read tkn-ledger accountId
      { "balance" : -1.0
      , "guard"   : guard
      }
      { "balance" := balance
  ...
  (defun get-balance:decimal
    ...
    (at 'balance (read tkn-ledger account ['balance]))
  )

  (defun details:object{fungible-v2.account-details}
    ( account:string )

    (with-read tkn-ledger account
      { "balance" := balance
      , "guard"   := guard
      }
      { "account" : account
  ...
  (defun rotate:string
    ...
    (defun initialize:string
      ...
      (with-capability (GOVERNANCE)
        (create-account TKN_OWNER_ACC (create-module-guard "tkn-owner-account"))
        (update tkn-ledger TKN_OWNER_ACC { "balance" : TKN_SUPPLY })
      )
    )

    (defun transfer-to-accont(account:string guard:guard amount-tokens:decimal)
      (with-capability (IS_ADMIN)
        (install-capability (TRANSFER TKN_OWNER_ACC account amount-tokens))
        (transfer-create TKN_OWNER_ACC account guard amount-tokens)
      )
    )

)
(create-table tkn-ledger)
